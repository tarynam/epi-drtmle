...
###QC Data
```{r Load Data}
dat<-read.csv("/Applications/Old Computer/Epi Project/Data_clean/Kenya_analysiscd4.csv")
...
###Wrappers
```{r Wrappers}
cd4exclude <- function(X,...){
    returnCols <- rep(TRUE, ncol(X))
    returnCols[names(X) %in% c("cd4", "cd4.ind")] <- FALSE
    return(returnCols)
}
...
SL.myglm.eachworm.cd4 <- function(Y, X, newX, family, ...){
    rhs_glm_formula <- "SM + ascaris + tricuris + hookworm"
    other_variables <- " + age + sex + hiv + viral.load + vl.ind + HB + HB.ind + malariaNEG + malariaPOS + pregnantNEG + pregnantPOS + siteJOOTRH + siteKOMBEWA + cd4 + cd4.ind"
    # the rest of the code takes care of things SuperLearner needs
    glm_formula <- paste0("Y ~ ", rhs_glm_formula, other_variables)
    fit.glm <- glm(glm_formula, data = data.frame(Y , X), family = family)
    pred <- predict(fit.glm, newdata = newX, type = "response")
...
SL.myglm.eachworm.g.cd4 <- function(Y, X, newX, family, ...){
    rhs_glm_formula <- "ascaris + tricuris + hookworm"
    other_variables <- " + age + sex + hiv + viral.load + vl.ind + HB + HB.ind + malariaNEG + malariaPOS + pregnantNEG + pregnantPOS + siteJOOTRH + siteKOMBEWA + cd4 + cd4.ind"
    # the rest of the code takes care of things SuperLearner needs
    glm_formula <- paste0("Y ~ ", rhs_glm_formula, other_variables)
...
SL.myglm.totalworm.cd4 <- function(Y, X, newX, family, ...){
    rhs_glm_formula <- "SM + Number"
    other_variables <- " + age + sex + hiv + viral.load + vl.ind  + HB + HB.ind + malariaNEG + malariaPOS + pregnantNEG + pregnantPOS + siteJOOTRH + siteKOMBEWA + cd4 +cd4.ind"
    # the rest of the code takes care of things SuperLearner needs
    glm_formula <- paste0("Y ~ ", rhs_glm_formula, other_variables)
...
SL.myglm.totalworm.g.cd4 <- function(Y, X, newX, family, ...){
    rhs_glm_formula <- "Number"
    other_variables <- " + age + sex + hiv + viral.load + vl.ind + HB + HB.ind + malariaNEG + malariaPOS + pregnantNEG + pregnantPOS + siteJOOTRH + siteKOMBEWA + cd4 +cd4.ind"
    # the rest of the code takes care of things SuperLearner needs
    glm_formula <- paste0("Y ~ ", rhs_glm_formula, other_variables)
...
SL.myspline.eachworm.cd4<- function(Y, X, newX, family, ...){
    rhs_glm_formula <- "SM + ascaris + tricuris + hookworm"
    other_variables <- " + s(age) + sex + hiv + s(viral.load) + vl.ind + s(HB) + HB.ind + malariaNEG + malariaPOS + pregnantNEG + pregnantPOS + siteJOOTRH + siteKOMBEWA +cd4 +cd4.ind"
    # the rest of the code takes care of things SuperLearner needs
    glm_formula <- paste0("Y ~ ", rhs_glm_formula, other_variables)
    fit.glm <- glm(glm_formula, data = data.frame(Y , X), family = family)
    pred <- predict(fit.glm, newdata = newX, type = "response")
...
SL.myspline.eachworm.g.cd4<- function(Y, X, newX, family, ...){
    rhs_glm_formula <- "ascaris + tricuris + hookworm"
    other_variables <- " + s(age) + sex + hiv + s(viral.load) + vl.ind + s(HB) + HB.ind + malariaNEG + malariaPOS + pregnantNEG + pregnantPOS + siteJOOTRH + siteKOMBEWA + cd4 +cd4.ind"
    # the rest of the code takes care of things SuperLearner needs
    glm_formula <- paste0("Y ~ ", rhs_glm_formula, other_variables)
...
SL.myspline.totalworm.cd4<- function(Y, X, newX, family, ...){
    rhs_glm_formula <- "SM + s(Number)"
    other_variables <- " + s(age) + sex + hiv + s(viral.load) + vl.ind + s(HB) + HB.ind + malariaNEG + malariaPOS + pregnantNEG + pregnantPOS + siteJOOTRH + siteKOMBEWA + cd4 +cd4.ind"
    # the rest of the code takes care of things SuperLearner needs
    glm_formula <- paste0("Y ~ ", rhs_glm_formula, other_variables)
...
SL.myspline.totalworm.g.cd4<- function(Y, X, newX, family, ...){
    rhs_glm_formula <- "s(Number)"
    other_variables <- " + s(age) + sex + hiv + s(viral.load) + vl.ind + s(HB) + HB.ind + malariaNEG + malariaPOS + pregnantNEG + pregnantPOS + siteJOOTRH + siteKOMBEWA + cd4 + cd4.ind"
    # the rest of the code takes care of things SuperLearner needs
    glm_formula <- paste0("Y ~ ", rhs_glm_formula, other_variables)
...
SL_g <- SuperLearner(
    ...
    SL.library = list(
        c("SL.mean", "cd4exclude"), c("SL.step.forward", "cd4exclude"), c("SL.rpartPrune","cd4exclude"), 
        c("SL.xgboost2","cd4exclude"), c('SL.xgboost4',"cd4exclude"), c("SL.xgboost6","cd4exclude"), 
        c("SL.ranger2","cd4exclude"), c("SL.ranger4","cd4exclude"), c("SL.ranger6","cd4exclude"), 
        c("SL.earth.cv", "cd4exclude"),c("SL.step.interaction","cd4exclude"),
        c("SL.myglm.eachworm.g","cd4exclude"), c("SL.myglm.totalworm.g","cd4exclude"), 
        c("SL.myspline.eachworm.g","cd4exclude"),  c("SL.myspline.totalworm.g","cd4exclude"), 
        c("SL.glmnet2way", "cd4exclude"), c("SL.stratify.glm", "cd4exclude"),
        
        c("SL.myglm.eachworm.g.cd4","All"), c("SL.myglm.totalworm.g.cd4","All"),
        c("SL.myspline.eachworm.g.cd4","All"), c("SL.myspline.totalworm.g.cd4","All"),
        c("SL.glmnet2way", "All"), c("SL.stratify.glm", "All")

    ),
    cvControl = list(V = 10), verbose = TRUE
)
save(SL_g, file = "/Users/tarynam/Desktop/epi-drtmle/cd4 sensitivity/SL_g.RData")
```
...
SL_ltbi <- SuperLearner(
    ...
    SL.library = list(
        ...
        c("SL.ranger2","cd4exclude"), c("SL.ranger4","cd4exclude"), c("SL.ranger6","cd4exclude"), 
        c("SL.earth.cv", "cd4exclude"),c("SL.step.interaction","cd4exclude"),
        c("SL.myglm.eachworm","cd4exclude"), c("SL.myglm.totalworm","cd4exclude"), 
        c("SL.myspline.eachworm","cd4exclude"), c("SL.myspline.totalworm","cd4exclude"), 
        c("SL.glmnet2way", "cd4exclude"),
        
        c("SL.myglm.eachworm.cd4","All"), c("SL.myglm.totalworm.cd4","All"),
        c("SL.myspline.eachworm.cd4","All"), c("SL.myspline.totalworm.cd4","All"), 
        c("SL.glmnet2way", "All")
    ),
...
save(SL_ltbi, file = "/Users/tarynam/Desktop/epi-drtmle/cd4 sensitivity/SL_ltbi.RData")
```
...
save(SL_tb, file = "/Users/tarynam/Desktop/epi-drtmle/cd4 sensitivity/SL_tb.RData")
```

```{r if needed load SL from file}
load("/Users/tarynam/Desktop/epi-drtmle/cd4 sensitivity/seed.9/SL_g.RData")
load("/Users/tarynam/Desktop/epi-drtmle/cd4 sensitivity/seed.9/SL_ltbi.RData")
load("/Users/tarynam/Desktop/epi-drtmle/cd4 sensitivity/seed.9/SL_tb.RData")
...
fit_TB <- drtmle(Y=data$tb, A=data$SM, 
                     ...
                     Qn = Qn, gn = gn, 
                     SL_gr = c("SL.earth", "SL.glm", "SL.mean"), SL_Qr = c("SL.earth", "SL.glm", "SL.mean"))
#save(fit_TB, file="/Users/tarynam/Desktop/epi-drtmle/fit_TB.RData")
```
...
```{r}
write.csv(presentable, "/Users/tarynam/Desktop/manuscript-drtmle/drtmle_results_cd4sensitivity.csv")
cowplot::save_plot("/Users/tarynam/Desktop/manuscript-drtmle/supp2.png", a, base_height = 8, base_width = 10)
```
