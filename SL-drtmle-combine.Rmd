...
SL_A <- SuperLearner(
    ...
    # the example using glm had the SL_A return a vector the full length of the data set
...
ps_A1 <- SL_A$SL.predict
# propensity for A = 0
ps_A0 <- 1 - ps_A1

# now regress DeltaY on A + W | DeltaA = 1. Here we are pooling over values of A
# this is what drtmle does if stratify = FALSE
SL_DeltaY <- SuperLearner(
...
ps_DeltaY_A1 <- predict(SL_DeltaY, newdata=dat1)$pred #then predict on this using the SL output

# get estimated propensity for observing outcome if A = 0 ... same as above but with A = 0
dat0 <- dplyr::select(dat, -SM) 
dat0$SM <- 0
ps_DeltaY_A0 <- predict(SL_DeltaY, newdata=dat0)$pred

# since we want to run drtmle on HIV- and HIV+ separately we need a way to
# only include propensity scores for each group
include<-rownames(subset(dat, dat$HIV==0))
# propensity for observing A, A =1 and for observing Y given A = 1
gn <- data.frame(ps_DeltaA * ps_A1 * ps_DeltaY_A1)
gn <- subset(gn, rownames(gn) %in% include)
gn_1 <- gn$ps_DeltaA...ps_A1...ps_DeltaY_A1
# propensity for observing A, A = 0 and for observing Y given A = 0
gn <- data.frame(ps_DeltaA * ps_A0 * ps_DeltaY_A0)
gn <- subset(gn, rownames(gn) %in% include)
gn_0 <- gn$ps_DeltaA...ps_A0...ps_DeltaY_A0
...
# ---> Error in SuperLearner(Y = dat[, y], X = dplyr::select(dat, -y), family = binomial(),  : 
...
# pass in this gn to drtmle ... what about Qn?
fit_HIVneg <- drtmle(Y=data[,y], A=data$SM, 
                     W=dplyr::select(data, -LTBI, -SM),
                     a_0 = c(1,0), family = binomial(),
                     Qn = Qn10, gn = gn10, 
                     SL_gr = c("SL.earth", "SL.glm"), SL_Qr = c("SL.earth", "SL.glm")
