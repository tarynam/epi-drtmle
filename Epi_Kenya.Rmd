...
QFT1b <- filter(QFT1b, !(StudyID %in% inter))%>%
    ...
    filter(TBAntigen < 10 | Mitogen < 10)%>%
    mutate(Study="NKS")%>%
    select(RecID , StudyID , ResultDate , NIL , TBAntigen , Mitogen, UserCode, EntryDate, Created , Modified , RowVersion)
l<-list(QFT1a, QFT1b)
...
```{r demographic and clinical data both}
library(dplyr)
setwd("/Applications/Old Computer/Epi Project/Data_original/")
...
both<-dplyr::rename(both, IgG.both = IgG, OD.both =OD)
...
dfs<-list(TBRU, Cohort, both, Enrollment, xray)
...
sputum.missing<-select(Total, StudyID, TB, HIV, CXR.REPORT, FM..RESULTS, EXPERT.RESULTS, HAIN.TEST.RESULTS, MGIT.Culture, Days.to.MGIT.Culture.)%>%
    filter(is.na(TB)|TB=="ACTIVE"|HIV=="POS")%>%
    filter(is.na(FM..RESULTS)|is.na(EXPERT.RESULTS)|is.na(HAIN.TEST.RESULTS)|is.na(MGIT.Culture))

QFT.missing<-select(Total, StudyID, TB, NIL, TBAntigen, Mitogen, QFT)%>%
    filter(is.na(QFT))

chest.missing<-select(Total, StudyID, TB, CXR.REPORT, Days.on.TB.Tx, Household.contact.treated.for.TB.within.past.2.yrs, Treated.for.TB.Previously.)%>%
    filter(is.na(TB)|is.na(CXR.REPORT)|is.na(Household.contact.treated.for.TB.within.past.2.yrs))
