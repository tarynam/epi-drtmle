...
library(dplyr)
setwd("/Applications/Old Computer/Epi Project/Data_original/")
QFT1 <- read.csv("TBRU & NK QFT Results_2016_DEC_19.csv")
...
Total<-select(Total, -DATE.CONSENTED, -ResultDate, -EntryDate, -Created, -Modified, -RowVersion,
              -DateFormFilled, -HIVtestDate, -DateQCDone
              )
Total<-unique(Total)
...
```{r updating and including location data}
...
```{r Seoarate into Data Tables to get missing data}
#Missing values
library(dplyr)
...
myList <-list(Dates, TB.missing, Hel.missing, Demographic.missing, HIV.missing)
for(i in names(myList)){
  write.csv(myList[[i]], paste0(i,".csv"))
}

write.csv(Total, paste(paste("total", date(), sep="_"),"csv", sep="."))
